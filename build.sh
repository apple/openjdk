...
if [[ -z "${BOOTSTRAP_JDK}" ]] ; then
    if [[ -d /AppleInternal/Java/Home/14 ]] ; then
        BOOTSTRAP_JDK=/AppleInternal/Java/Home/14
    elif [[ -d /AppleInternal/Java/Home/13 ]] ; then
...
for subdir in bin lib ; do
    pushd ${UNIVERSAL_JDK}/Contents/Home/${subdir}

    find . -type f | while read file ; do
        FILE_TYPE=$(file "${file}")
        [[ ${FILE_TYPE} =~ "Mach-O" ]] || continue

        if [[ -f "${ARM64_JDK}/Contents/Home/${subdir}/${file}" && -f "${X86_64_JDK}/Contents/Home/${subdir}/${file}" ]] ; then
            lipo -output ${file} -create ${ARM64_JDK}/Contents/Home/${subdir}/${file} ${X86_64_JDK}/Contents/Home/${subdir}/${file}
        fi
    done
done
